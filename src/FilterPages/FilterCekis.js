...
const FilterCekis = ({ navigation }) => {
  const { setFilter, CekisFiltre, setCekisFiltre } = useContext(FilterContext);
  let Cekisler = ["Önden Çekiş", "Arkadan İtiş"]
  const [secilen, setSecilen] = useState([])
  let markacheck = []
  return (
    <View style={styles.container}>
      <Text style={styles.header}>Filtrelemek İstediğiniz Çekiş Tipini Seçin</Text>
      <ScrollView style={{ width: "80%" }} showsVerticalScrollIndicator={false}>
        {

          Cekisler.map((element, value) => (
